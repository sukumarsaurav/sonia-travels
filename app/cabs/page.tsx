...
const WHY_US = [
  { icon: '✅', title: 'Verified & trained drivers', desc: 'All drivers are police-verified, licensed and familiar with mountain routes.' },
  { icon: '📍', title: 'GPS-tracked vehicles', desc: 'Real-time tracking on every cab. Share your live location with family.' },
  { icon: '🕐', title: 'On-time guarantee', desc: 'We start tracking your flight/train and adjust pickup time automatically.' },
  { icon: '💰', title: 'Fixed transparent fares', desc: 'Price quoted is price paid. No surge, no hidden charges, no waiting fees.' },
  { icon: '📞', title: '24×7 helpline', desc: 'Call or WhatsApp any time — before, during or after your journey.' },
  { icon: '🏔️', title: 'Mountain road expertise', desc: 'Our drivers handle Himachal & J&K routes including Rohtang, Jalori, Sinthan.' },
]
        ...
        <div style={{ background: 'linear-gradient(135deg, var(--ink-900) 0%, #1a2a1a 100%)', paddingTop: 96, paddingBottom: 80, color: 'white' }}>
          <div style={{ maxWidth: 1240, margin: '0 auto', padding: '0 32px' }}>
            ...
            <p style={{ fontSize: 18, color: 'rgba(255,255,255,0.75)', margin: '0 0 32px', maxWidth: 560, lineHeight: 1.6 }}>
              Outstation taxis to Dalhousie, Dharamshala, Amritsar, Jammu, Katra, Manali &amp; beyond. AC sedans, SUVs and Tempo Travellers. Verified drivers. Fixed fares.
            </p>
            ...
            <div style={{ display: 'flex', gap: 32, marginTop: 40, paddingTop: 32, borderTop: '1px solid rgba(255,255,255,0.1)' }}>
              {[['10,000+', 'trips completed'], ['4.8★', 'average rating'], ['24×7', 'helpline'], ['0', 'hidden charges']].map(([n, l]) => (
                <div key={l}>
            ...
            {POPULAR_ROUTES.map((route, i) => (
              <Link key={route.slug} href={`/cabs/${route.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                <div className="lift reveal" style={{ background: 'white', border: '1px solid var(--line)', borderRadius: 14, padding: 24, transitionDelay: `${i * 0.06}s`, cursor: 'pointer' }}>
                  ...
                  <div style={{ marginTop: 16, fontSize: 13, fontWeight: 600, color: 'var(--terra-700)', display: 'flex', alignItems: 'center', gap: 4 }}>
                    View details &amp; book →
                  </div>
                </div>
              </Link>
                  ...
                  {ALL_ROUTES.map((r, i) => (
                    <tr key={r.slug} style={{ borderBottom: '1px solid var(--line)', background: i % 2 === 0 ? 'white' : 'var(--sand-50)' }}>
                      ...
                      <td style={{ padding: '14px 16px', textAlign: 'center' }}>
                        <Link href={`/cabs/${r.slug}`} style={{ fontSize: 12, fontWeight: 700, color: 'var(--terra-700)', textDecoration: 'none', whiteSpace: 'nowrap' }}>
                          Book →
                        </Link>
                      </td>
                    </tr>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }} className="grid-3">
            {WHY_US.map((w, i) => (
              <div key={i} className="reveal" style={{ padding: '24px 20px', background: 'var(--sand-50)', border: '1px solid var(--line)', borderRadius: 14, transitionDelay: `${i * 0.07}s` }}>
                ...
                <div style={{ fontFamily: 'var(--serif)', fontSize: 20, fontWeight: 500, marginBottom: 6 }}>{w.title}</div>
                <div style={{ fontSize: 13, color: 'var(--ink-700)', lineHeight: 1.55 }}>{w.desc}</div>
              </div>
            ))}
          </div>
        </Section>
        ...
        <div style={{ background: 'var(--terra-700)', color: 'white', padding: '64px 32px' }}>
          <div style={{ maxWidth: 1240, margin: '0 auto', display: 'grid', gridTemplateColumns: '1.4fr auto', gap: 40, alignItems: 'center' }} className="cta-strip">
            <div>
              ...
              <p style={{ fontSize: 16, opacity: 0.85, margin: 0 }}>Call or WhatsApp us — we'll confirm your cab within minutes. No app needed.</p>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <a href="tel:+918460222809" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: 'white', color: 'var(--terra-800)', padding: '14px 28px', borderRadius: 8, fontWeight: 700, fontSize: 15, textDecoration: 'none', whiteSpace: 'nowrap' }}>
                📞 +91 84602 22809
              </a>
              <a href="https://wa.me/918460222809?text=Hi%2C+I+need+a+cab+from+Pathankot" target="_blank" rel="noopener noreferrer"
          ...
          <div style={{ maxWidth: 760, display: 'grid', gap: 0 }}>
            {[
              ...
              <div key={i} style={{ padding: '20px 0', borderBottom: '1px solid var(--line)' }}>
                <div style={{ fontFamily: 'var(--serif)', fontSize: 18, fontWeight: 500, marginBottom: 8 }}>Q: {f.q}</div>
