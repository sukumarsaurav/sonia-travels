...
export function PackageCard({ pkg, featured }: { pkg: Package; featured?: boolean }) {
  return (
    <Link href={`/packages/${pkg.id}`} className="lift press reveal" style={{
      ...
      <div className={`ph-img ${pkg.hero}`} style={{ height: featured ? 280 : 200, position: 'relative' }}>
      ...
      <div style={{ padding: 20 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--ink-700)', margin: '0 0 16px', lineHeight: 1.5 }}>{pkg.desc}</p>
